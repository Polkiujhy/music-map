...
- FR-004: Zalogowany użytkownik może zaimportować do banku `music-map` playlistę z linku Spotify albo YouTube, jeżeli platforma pozwala odczytać jej zawartość. Spotify wymaga OAuth użytkownika i obsługuje wyłącznie playlistę należącą do niego lub taką, której jest współpracownikiem. YouTube pozwala odczytać playlistę publiczną albo `unlisted` przez API key, a prywatną po OAuth właściciela. Przy odmowie użytkownik otrzymuje przyczynę i wykonalne rozwiązanie. Priority: must-have
  > Sokrates: Rozważono, że share link nie nadaje dostępu do elementów playlisty. Rozwiązanie: Spotify nie obiecuje importu arbitralnej cudzej playlisty, nawet publicznej; YouTube rozdziela publiczny odczyt od dostępu do danych prywatnych, a odmowa jest pełnoprawnym wynikiem produktu.
...
- FR-009: Użytkownik z powiązanym kontem platformy może utworzyć eksportowaną playlistę bezpośrednio na swoim koncie albo zaktualizować jej wcześniej zapisaną kopię, rozpoznawaną wyłącznie po ID playlisty nadanym przez platformę. YouTube tworzy wynik jako `unlisted`. Spotify tworzy wynik z `public=false`: niewidoczny w profilu i wyszukiwarce, ale bez gwarancji kontroli dostępu dla osoby posiadającej link. Priority: must-have
  > Sokrates: Rozważono niepotrzebne ujawnienie kolekcji i różne gwarancje platform. Rozwiązanie: YouTube używa trwałego linku `unlisted`, a Spotify nie przedstawia `public=false` jako ścisłej prywatności.
- FR-010: Użytkownik bez powiązanego konta może otrzymać playlistę utworzoną na koncie technicznym `music-map` albo zaktualizować jej wcześniej zapisaną kopię, rozpoznawaną wyłącznie po ID playlisty nadanym przez platformę. Wynik używa `unlisted` na YouTube albo `public=false` na Spotify, jest dostępny przez stały link i nie jest publikowany w profilu ani wyszukiwarce; Spotify nie gwarantuje jednak kontroli dostępu do linku. Wynik zawiera link, instrukcję, informację o właścicielu i miejscu edycji. Priority: must-have
