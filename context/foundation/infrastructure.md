...
- **Approval**: A human approves production publishing, the four immutable image digests, initial or changed database migrations, the baseline fingerprint, secret rotation, public routing changes, database deletion, and rollback when data compatibility is uncertain. An agent may run tests, inspect configuration, perform `s-manager --dry-run deploy`, and read bounded status/log output unattended.
...
| CI builds but does not publish deployable image digests or a manifest | Unknown unknowns | H | M | Add a later reviewed release job that publishes the four exact images, records digests/source SHA, and emits the strict schema-version-1 manifest. |
...
1. Verify the already provisioned production inputs without changing them: `s-manager provision music-map --status`, then `s-manager status music-map --json`. The expected starting state is a dedicated `music_map` database with `schema: not_initialized` and application state `not_deployed`.
2. Run the version-pinned application checks: `composer test`, `vendor/bin/pint --test`, and `npm run build`. Build and publish the existing Dockerfile targets `fpm`, `nginx`, `queue`, and `scheduler` with the exact 40-character source SHA, then record each immutable registry digest in a schema-version-1 release manifest. Do not substitute mutable tags.
3. Initialize the dedicated PostgreSQL database with the exact Laravel 13 migration tree through a separately reviewed one-off run of `php artisan migrate --force`. Verify the resulting schema independently and set `baseline_migration_fingerprint` only to the fingerprint for that already-applied migration set; ordinary deploy and rollback commands must remain unable to apply migrations.
4. Validate the manifest and expected state: `s-manager --dry-run deploy music-map --release-file /path/to/release.json --expected-current none`. Then deploy with `s-manager deploy music-map --release-file /path/to/release.json --expected-current none`.
5. Require `s-manager status music-map --json` to report all four roles healthy and HTTP-ready, inspect `s-manager logs music-map --no-follow --tail 100 --timestamps`, then replace the intentional `music.adamis.me` 503 placeholder with the reviewed public-edge proxy and run an external HTTPS smoke test.
